...
<body>
  ...
  <?php
     $data=0;
     $sql="SELECT * FROM category where category='Product-Launches' and is_active=0";
     $result=$pdo->query($sql);
     $data=$result->fetch();
     if(empty($data)){
        $result=$pdo->query("SELECT * FROM subcategory where sub_category='Product-Launches' and is_active=0");
        $data=$result->fetch();
     }
   ?>
  <!--<section>-->
...
<!-- !advertisment area -->
      ...
      <div class="col-lg-8 col-6 px-lg-5 px-0">
        ...
        <div class="row g-3">
          <?php $sql="SELECT * FROM news where (category='Product-Launches' or sub_category='Product-Launches') and is_active=0";
           $result=$pdo->query($sql);
          $data=$result->fetchAll();
          foreach ($data as $value) {
              $a = strip_tags( $value['heading']);
